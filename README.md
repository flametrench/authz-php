...
**Status:** v0.3.0 (stable). PHP 8.3+ required. Includes `ShareStore` ([ADR 0012](https://github.com/flametrench/spec/blob/main/decisions/0012-share-tokens.md)) and Postgres-backed adapters (`PostgresTupleStore`, `PostgresShareStore`); v0.3 adds Postgres-backed rewrite-rule evaluation ([ADR 0017](https://github.com/flametrench/spec/blob/main/decisions/0017-postgres-rewrite-rule-evaluation.md)) and the relaxed `tup.subject_type` constraint required for `pat`-subject tuples and adopter-defined hop types. Per [ADR 0013](https://github.com/flametrench/spec/blob/main/decisions/0013-postgres-adapter-transaction-nesting.md) the Postgres adapters cooperate with adopter-side outer transactions via savepoints when nested. Upgrading from v0.2? See [`docs/migrating-to-v0.3.md`](https://github.com/flametrench/spec/blob/main/docs/migrating-to-v0.3.md) — the schema migration is one `ALTER TABLE`.
